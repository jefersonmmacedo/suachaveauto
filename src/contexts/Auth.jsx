...
function AuthProvider({children}) {
    ...
    async function createAccount({
        ...
        await api.post('/client', data).then(() => {
          //  completeAccount(email)
            ...
            localStorage.setItem("suachaveauto", JSON.stringify(data));
    ...
    async function loginSession({email, password}) {     
            await api.post("/session/client", {email, password}).then((result) => {
                ...
                localStorage.setItem("suachaveauto", JSON.stringify(result.data));
...
// Fim da Sessão grupos
    ...
    async function newContact({
        ...
        const data = {idAuto, idCompany, nameCompany, idClient, name, email, phone, whatsapp, origin, type, latitude, longitude}
        ...
        await api.post("/contact/", data).then((res) => {
            ...
            if(type === "Ligação") {
                ...
                window.open(`https://api.whatsapp.com/send?phone=55${whatsappCompany}&text=Olá. Me chamo ${name}, gostaria de saber mais detalhes sobre o anúncio:%20 ${link}`)
            }
    ...
    async function newContactCompany({
        idAuto, idCompany, nameCompany, idClient, name, email, phone, whatsapp, type, whatsappCompany, phoneCompany
    }) {
        const data = {idAuto, idCompany, nameCompany, idClient, name, email, phone, whatsapp, type}
    ...
    async function handleNewAlertClient({idAuto, name, email, whatsapp, status, district, city, uf, type, subType, bedroom, suite, restroom, garage, pets, furnished}) {
        const data = {idAuto, name, email, whatsapp, status, district, city, uf, type, subType, bedroom, suite, restroom, garage, pets, furnished}
    ...
    async function handleNewSearchClient({name, email, whatsapp, status, district, city, uf, type, subType, bedroom, suite, restroom, garage, pets, furnished}) {
        ...
        toast.success("Busca de anúncio cadastrado com sucesso!")
  ...
  // Deslogandop após tempo de inatividade
   function inactivityTime() {
       ...
       function doSomething() {
        const DataUser = localStorage.getItem("suachaveauto");
