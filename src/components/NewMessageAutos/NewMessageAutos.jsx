...
export function NewMessageAutos({idAuto, idCompany, imageAuto}) {
  ...
  const {data} = useFetch(`/rooms/${idAuto}/${idCompany}/${user?.id}`);
  ...
  function handleNewChatMessage() {
      if(data?.length > 0) {
        window.open(`/chat/${data[0]?.room}/${idAuto}/${idCompany}/${user?.id}`, "_self");
        return
      }
  ...
  async function handleCreateNewChatmenssage() {
    ...
    await api.post("/rooms", newRoom).then((res) => {
      window.open(`/chat/${newRoom.id}/${newRoom.idAuto}/${newRoom.idCompany}/${newRoom.idClient}`, "_self");
    }).catch((err) => {
      console(err);
    });
  }
