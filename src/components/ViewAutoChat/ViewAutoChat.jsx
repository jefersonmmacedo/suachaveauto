...
export function ViewAutoChat({id, mobile}) {
...
const payments = valuesRent?.reduce(function (acumulador, objetoAtual){
  ...
  var ResultBRL = payments.toLocaleString('pt-br',{style: 'currency', currency: 'BRL'})
    ...
    return (
        <div className="ViewAutoChat">
            {mobile === true ? 
  <button className="btnAuto2" onClick={handleFiltro}><IoHome/> Ver Imóvel</button>
            : mobile === false ? 
            <button className="btnAuto" onClick={handleFiltro}><IoHome/> Ver Imóvel</button>
            : mobile === "List" ?
            <button className="btnAuto3" onClick={handleFiltro}><IoHome/> Ver Imóvel</button>
            :
  <button className="btnAuto" onClick={handleFiltro}><IoHome/> Ver Imóvel</button>
            }
